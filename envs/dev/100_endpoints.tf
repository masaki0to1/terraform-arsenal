...
module "vpce_rds" {
  ...
  service_name        = "com.amazonaws.${local.default_region}.rds"
...
module "vpce_ssm" {
  ...
  vpc_id              = module.vpc_core.vpc.id
  vpc_endpoint_type   = "Interface"
  service_name        = "com.amazonaws.${local.default_region}.ssm"
  subnet_ids          = [for pri_subnet in module.vpc_core.private_subnets : pri_subnet.id]
  security_group_ids  = [module.endpoint_sg.attrs.id]
  private_dns_enabled = true
}
...
module "vpce_ssm_messages" {
  ...
  vpc_id              = module.vpc_core.vpc.id
  vpc_endpoint_type   = "Interface"
  service_name        = "com.amazonaws.${local.default_region}.ssmmessages"
  subnet_ids          = [for pri_subnet in module.vpc_core.private_subnets : pri_subnet.id]
  security_group_ids  = [module.endpoint_sg.attrs.id]
  private_dns_enabled = true
}
...
module "vpce_ecr_api" {
  ...
  vpc_id              = module.vpc_core.vpc.id
  vpc_endpoint_type   = "Interface"
  service_name        = "com.amazonaws.${local.default_region}.ecr.api"
  subnet_ids          = [for pri_subnet in module.vpc_core.private_subnets : pri_subnet.id]
  security_group_ids  = [module.endpoint_sg.attrs.id]
  private_dns_enabled = true
}
...
module "vpce_ecr_dkr" {
  ...
  vpc_id              = module.vpc_core.vpc.id
  vpc_endpoint_type   = "Interface"
  service_name        = "com.amazonaws.${local.default_region}.ecr.dkr"
  subnet_ids          = [for pri_subnet in module.vpc_core.private_subnets : pri_subnet.id]
  security_group_ids  = [module.endpoint_sg.attrs.id]
  private_dns_enabled = true
}
...
module "vpce_cw_logs" {
  ...
  vpc_id              = module.vpc_core.vpc.id
  vpc_endpoint_type   = "Interface"
  service_name        = "com.amazonaws.${local.default_region}.logs"
  subnet_ids          = [for pri_subnet in module.vpc_core.private_subnets : pri_subnet.id]
  security_group_ids  = [module.endpoint_sg.attrs.id]
  private_dns_enabled = true
}
