...
module "private_alb_sg" {
  ...
  vpc_id      = module.vpc_core.vpc.id
  description = "Allow TLS all inbound traffic from VPC CIDR ${module.vpc_core.vpc.cidr} and all outbound traffic."
  ...
  ingress_rules = [
    {
      ...
      description = "Allow HTTP from VPC CIDR ${module.vpc_core.vpc.cidr}"
    },
    {
      from_port   = 443
      to_port     = 443
      protocol    = "tcp"
      cidr_blocks = [module.vpc_core.vpc.cidr]
      description = "Allow HTTPS from VPC CIDR ${module.vpc_core.vpc.cidr}"
    }
  ]
...
module "rds_sg" {
  ...
  vpc_id      = module.vpc_core.vpc.id
  description = "Allow TLS inbound traffic to port 3306 from VPC CIDR ${module.vpc_core.vpc.cidr} and all outbound traffic."
  ...
  ingress_rules = [
    {
      ...
      protocol    = "tcp"
      cidr_blocks = [module.vpc_core.vpc.cidr]
      description = "Allow inbound traffic to port 3306 from VPC CIDR ${module.vpc_core.vpc.cidr}"
    },
    {
      from_port       = 3306
      to_port         = 3306
...
module "endpoint_sg" {
  ...
  vpc_id      = module.vpc_core.vpc.id
  description = "Allow TLS inbound traffic to port 443 from VPC CIDR ${module.vpc_core.vpc.cidr} and all outbound traffic"
