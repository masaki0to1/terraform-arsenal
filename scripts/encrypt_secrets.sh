...
PLAIN_TEXT=$(echo "$input" | jq -r '.plain_text')
...
if [ -n "${PREFIX}" ]; then
  ...
  echo "Failed to encrypt: PREFIX is missing" >&2
  exit 1
fi
...
if [ -z "${KEY_ALIAS}" ]; then
  echo "Failed to encrypt ${PREFIX}: KMS Key ID is missing" >&2
  exit 1
fi

# Encrypt the plain text
ENCB64=$(aws kms encrypt --key-id "${KEY_ALIAS}" --plaintext fileb://<(echo -n  "${PLAIN_TEXT}") --output text --query CiphertextBlob ${PROFILE_CONTEXT})
if [ $? -ne 0 ]; then
  echo "Failed to encrypt ${PREFIX}" >&2
  exit 1
fi
...
jq -n --arg key "encrypted_${PREFIX}" --arg value "${ENCB64}" '{($key): $value}'